...
/**
 * Every SecureStore key the app writes.
 *
 * SecureStore has no enumerate API, so this list has to be maintained by hand.
 * If you add a SecureStore key, add it here or a reset will silently leave it
 * behind.
 */
const SECURE_KEYS = [
  'tunematch_device_id',
 ...
 * `tunematch_guest_free_credits_granted_<deviceId>`, and SecureStore cannot list
 * keys, so the exact marker is not directly discoverable. Deleting
...
export async function resetAppToFreshInstall(): Promise<{ ok: boolean; steps: string[] }> {
  ...
  // 3) Everything in AsyncStorage: credits, guest history, taste profile, pro
  //    cache, the pro daily scan counters.
  try {
  ...
  // 4) SecureStore, including the suffixed grant markers we can still compute
  //    before the device id goes away.
  try {
    let suffixed: string[] = [];
    try {
      const deviceId = await getDeviceId();
      suffixed = [`tunematch_guest_free_credits_granted_${deviceId}`];
    } catch {
